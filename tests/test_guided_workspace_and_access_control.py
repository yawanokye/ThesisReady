...
def test_existing_complimentary_token_can_be_topped_up_and_extended_to_strengthener(monkeypatch, tmp_path):
    _use_temp_db(monkeypatch, tmp_path)
    import app.access_control as access

    issued = access.issue_complimentary_token(
        page_limit=10,
        label="Chapter 1 pilot",
        assigned_email="student@example.com",
        product_area="thesis_workspace",
        validity_days=10,
        created_by="test",
    ...
    reservation = access.reserve_complimentary_for_background(
        ...
        action="draft",
        requested_pages=8,
        idempotency_key="draft-ch1-token-continuity",
    )
    access.complete_complimentary_usage(reservation["complimentary_usage_id"])
    assert access.complimentary_status(token, product_area="thesis_workspace", supplied_email="student@example.com")["pages_remaining"] == 2

    updated = access.update_complimentary_token(
        issued["id"],
        add_pages=12,
        product_area="all",
        extend_days=30,
    ...
    assert updated["token"]["page_limit"] == 22
    assert updated["token"]["pages_remaining"] == 14
    assert updated["token"]["product_area"] == "all"

    strengthener = access.reserve_complimentary_for_background(
        raw_token=token,
        supplied_email="student@example.com",
        ...
        action="revision",
        requested_pages=10,
        idempotency_key="strengthen-ch1-same-token",
    )
    assert strengthener["reserved_pages"] == 10
    access.complete_complimentary_usage(strengthener["complimentary_usage_id"])
    status = access.complimentary_status(token, product_area="chapter_strengthener", supplied_email="student@example.com")
    assert status["pages_remaining"] == 4
